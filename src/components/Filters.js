          ...
          <div className="form-control">
            ...
            <div>
              {categories.map((cat,index)=>{
                return <button type="button" name='category' className={`${category===cat.toLowerCase()? 'active':null}`} onClick={updateFilters} key={index}>{cat}</button>
              })}
            </div>
          </div>
          {/*end of category*/}
           ...
           <div className="form-control">
            ...
            <div className="colors">
              {colors.map((col,index)=>{
                if(col==='all'){
                  if(col==='all'){
                  return <button type='button'  key={index} onClick={updateFilters} data-color="all" name="color" className={`${color==="all"? 'all-btn active':'all-btn'}`}>all</button>
                }
                }
                return <button type="button" style={{background:col}} name='color' className={`${color===col ? 'color-btn active':'color-btn'}`} onClick={updateFilters} key={index} data-color={col}>{color===col?<FaCheck />:null}</button>
              })}
            </div>
          </div>
            {/* end of colors*/}
